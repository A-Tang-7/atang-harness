...
- **被通配规则覆盖的子集条目**：如已有 `Bash(git *)`，则 `Bash(git add *)`、`Bash(git commit *)` 等会被自动剔除
...
// 去掉被更宽泛规则覆盖的条目（如 Bash(git *) 覆盖 Bash(git add *)）
function isCoveredByBroader(rule, rules) {
  // 只有通配规则（以 *) 结尾）才能覆盖其他规则
  const broadRules = rules.filter(r => r.endsWith('*)') && r !== rule);
  for (const broad of broadRules) {
    const prefix = broad.slice(0, -2); // 去掉末尾的 *)
    if (rule.startsWith(prefix)) return true;
  }
  return false;
}
const slim = unique.filter(r => !isCoveredByBroader(r, unique));
...
// 构建结果，保留原文件其他字段
const base = { permissions: { allow: slim } };
fs.writeFileSync(target, JSON.stringify(base, null, 2) + '\n');
console.log(`合并完成: ${allAllows.length} → ${filtered.length} → ${unique.length} → ${slim.length} 条（去重后去子集）`);
```
